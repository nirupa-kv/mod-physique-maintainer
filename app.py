...
# MODULE 2: 3D PHYSIQUE TRANSFORMER MODELLING SLIDERS
    ...
    st.write("---")
    st.subheader("🏋️‍♂️ Targeted Corrective Exercise Interventions")
    active_segment = "thighs" if thighs > 24.0 else ("arms" if arms > 14.0 else "chest")
    workouts = db.fetch_targeted_workouts(active_segment)
    
    if workouts:
        for wk in workouts:
            st.info(f"👉 **Exercise Challenge: {wk['exercise_name']}**")
            st.markdown(wk['description'])
...
# MODULE 5: REGIONAL CALORIC DIETARY LEDGER
    ...
    if foods:
        ...
        st.warning("No entries found matching this culinary subset.")
