# database.py
"""
100% Self-Contained Local Database Asset Hub
Bypasses cloud providers completely to run apps at lightning speed.
"""

# Static collection arrays mapped exactly from your master migration keys
LOCAL_FOOD_ENTRIES = [
...
LOCAL_WORKOUT_LIBRARY = [
    {"target_segment": "thighs", "exercise_name": "Goblet Squat Variants", "description": "3 sets of 12 reps. Targets deep quad fibers to address local thigh concentration."},
    {"target_segment": "arms", "exercise_name": "Tricep Overhead Extensions", "description": "3 sets of 15 reps. Tones the posterior arm layout."},
    {"target_segment": "chest", "exercise_name": "Incline Push-Ups / Dumbbell Press", "description": "3 sets of 12 reps. Optimizes tissue development across upper chest sections."}
]

def fetch_indian_foods(cuisine_type: str):
    """Filters local memory array entries instantly based on chosen regional culinary partitions."""
    return [food for food in LOCAL_FOOD_ENTRIES if food["cuisine_type"] == cuisine_type]

def fetch_targeted_workouts(segment: str):
    """Queries memory dictionary tracking for targeted body segment movements."""
    return [wk for wk in LOCAL_WORKOUT_LIBRARY if wk["target_segment"] == segment.lower()]